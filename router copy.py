...
!""".format(self.data[text_igp[0]][nb]["g3/0"], text_igp[1])
            ...
            if self.is_igp_ospf and self.router_num in self.data["bgp"]:
                  what_to_add += \
"""
router ospf 1
 router-id {}.{}.{}.{}""".format(nb, nb, nb, nb)

            if self.router_num in self.data["bgp"] and self.is_igp_ospf:
                  for key in self.data["bgp"][self.router_num].keys():
                  what_to_add += \
""" 
